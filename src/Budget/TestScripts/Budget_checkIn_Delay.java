...
'## BRAND:                                         AVIS
'## DESCRIPTION:                           Perform a Delayed CheckIn of Rental in different regions for BUDGET.
...
public class Budget_checkIn_Delay {
		...
		public void test() throws Exception
		{
			try
			{
			   ...
			   for (int K=1; K<=2; K++)
			   {
					...
					ReadWriteExcel rwe 		= new ReadWriteExcel("C:\\GUI_Testing\\TestData\\DelayedCheckIn\\BUDGET_GUITestData_DelayedCheckIn.xlsx");
					...
					String ScreenShotPath 	= "C:\\GUI_Testing\\ScreenShot\\Budget\\DelayedCheckIn\\";
					
					String Execute 			= rwe.getCellData("DelayedCheckIn_Budget", K,  1);
					String ENVIRONMENT 		= rwe.getCellData("DelayedCheckIn_Budget", K,  2);
					String TESTCASENAME		= rwe.getCellData("DelayedCheckIn_Budget", K,  3);
					String THINCLIENTURL	= rwe.getCellData("DelayedCheckIn_Budget", K,  5);
					String Region			= rwe.getCellData("DelayedCheckIn_Budget", K,  6);
					String USERID 			= rwe.getCellData("DelayedCheckIn_Budget", K,  7);
					String PASSWORD 		= rwe.getCellData("DelayedCheckIn_Budget", K,  8);
					String RANO				= rwe.getCellData("DelayedCheckIn_Budget", K,  9);
					String RETURNDATE		= rwe.getCellData("DelayedCheckIn_Budget", K, 10);
					String RETURNTIME		= rwe.getCellData("DelayedCheckIn_Budget", K, 11);
...
//							FileUtils.copyFile(src1,new File( ScreenShotPath +TESTCASENAME+ sdf1.format(d1)+"_Print"+".pgn"));
					...
					//Complete Check In
						...
						if (CompCheckIn.isDisplayed());
						{
						    ...
						    if (getCheckInText.contains("Complete Check In"))
						    {
								...
								rwe.setCellData("DelayedCheckIn_Budget", K, 12, CCI);
					...
					if (rwe.getCellData("DelayedCheckIn_Budget", K, 12).isEmpty())
					{
						rwe.setCellData("DelayedCheckIn_Budget", K, 13, "FAIL");
						test.log(Status.FAIL, "Fail");
					}
					else
					{
						test.log(Status.PASS, "Pass");
						rwe.setCellData("DelayedCheckIn_Budget", K, 13, "PASS");
					}
