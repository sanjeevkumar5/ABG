...
/**
 * '#############################################################################################################################
 * SCRIPT NAME      :  Budget_GUI_Vehicle_Exchange 
 * BRAND            :  Budget
...
public class Budget_GUI_Vehicle_Exchange
{
	...
	public static void main(String[] args) throws IOException, Exception, FileNotFoundException {
		...
		for (int k = 1; k <= 30; k++)
		{
//			GUI_Vehicle_Exchange_Budget Budget = new GUI_Vehicle_Exchange_Budget();
			ReadWriteExcel rwe = new ReadWriteExcel("C:\\Selenium\\TestData\\GUI_Budget_Vehicle_Exchange.xlsx");
...
//				String delayed     = rwe.getCellData("INPUT_DATA", k, 15);
				...
				else if (rwe.getCellData("INPUT_DATA", k, 11).equalsIgnoreCase("Off the Lot Exchange"))
				{
					...
					{
						functions.notdelayedoffthelot(mileageIn, prchsfuel, fuelin, adjstmnt, vehdamaged, accreport, fuelother, reason, amount, oneway, misc, miscfee, MVAnumber, Mileage, Fuel, VEremarks);
					}
				}
