...
public class Payless_GUI_CancelReservation
{
	...
	public void test() throws Exception 
	{
		...
		try {
			ReadWriteExcel rwe = new ReadWriteExcel("C:\\Selenium\\Test_Data\\PAYLESS\\Payless_GUITestData_CancelReservation.xlsx");
		...
		for (int k=1; k<=intRowCount;k++)
		{
			...
			if (Execute.equals("Y"))
			{
				...
				// functions.link(tokenURL,thinClient);
				 ...
				 driver.findElement(By.xpath("//input[@id='menulist:rateshopContainer:searchResForm:searchString']")).click();
				 driver.findElement(By.xpath("//input[@id='menulist:rateshopContainer:searchResForm:searchString']")).sendKeys(pResNo);
				 System.out.println("The Reservation Number is "+pResNo );
					...
					try {
						    if (driver.findElement(By.xpath("//a[@class='rateShopCancelLinkClass ng-binding']")).isDisplayed())
							{
							   ...
							   rwe.setCellData("Payless_GUI", k, 14, "Reservation Number is Cancelled Successfully");
							}			
					}
					...
					catch(Exception e)
					{
						...
						rwe.setCellData("Payless_GUI", k, 14, "Reservation Number already is Cancelled");
					}
